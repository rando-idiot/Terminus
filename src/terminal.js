...
export class Terminal {
  ...
  useCommands(commands) {
    if (!(commands instanceof Array)) commands = Object.values(commands);
  ...
  useCommand(command) {
    ...
    assert(
      isDefined(command.name) && isDefined(command.id),
      "Command must have a name and id."
    );
  ...
  fromJson(src) {
    ...
    for (const [name, hashes] of Object.entries(src.globalCommands).filter(
      ([n, _]) => n !== "class"
    )) {
      COMMANDS[name]
        ...
        ?.forEach(
          (cond, id) =>
            void console.warn(`Command ${serialize([name, id])} got changed.`)
        );
    }
...
export function Command(
  /** @type {(game: Game, ...args: (string | number)[]) => void>} */ fn
) {
  assert(
    NEW_COMMANDS_ALLOWED,
    "Creating commands after game initialization is forbidden"
  );
  ...
  let id =
    COMMANDS[fn.name]?.findIndex((f) => f.hash === fn.hash) ??
    ((COMMANDS[fn.name] = []), -1);
